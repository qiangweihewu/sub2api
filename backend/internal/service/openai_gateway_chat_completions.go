...
func (s *OpenAIGatewayService) forwardAsChatCompletions(
	...
	// Bedrock Mantle 账号强制走 CC 直转——ShouldUseResponsesAPI 默认 true，Mantle
	// 只有 /v1/chat/completions 端点，没有 /v1/responses，必须在此显式短路。
	if account.IsOpenAIBedrockMantle() || shouldForwardOpenAIResponsesViaRawChatCompletions(account) {
		return s.forwardAsRawChatCompletions(ctx, c, account, body, defaultMappedModel)
	}
