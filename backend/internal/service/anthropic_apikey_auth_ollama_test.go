...
func TestSetAnthropicAPIKeyAuthHeader_OllamaCloudForcesBearer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.wantXAPIKey, getHeaderRaw(header, "x-api-key"), "x-api-key 头不符")
		})
	}
}
...
func TestSetAnthropicAPIKeyAuthHeader_CNAdaptiveBaseURL(t *testing.T) {
	...
	require.Empty(t, getHeaderRaw(header, "x-api-key"))
	...
	require.Equal(t, "kimi-key", getHeaderRaw(header, "x-api-key"))
...
func TestGatewayService_AnthropicPassthrough_OllamaCloudBearer(t *testing.T) {
	...
	require.Empty(t, getHeaderRaw(ollamaReq.Header, "x-api-key"), "Ollama Cloud 上游不得再发 x-api-key")
	...
	require.Equal(t, "ollama-cloud-key", getHeaderRaw(anthropicReq.Header, "x-api-key"))
	require.NotContains(t, getHeaderRaw(anthropicReq.Header, "x-api-key"), "inbound-api-key")
}
...
func TestGatewayService_BuildUpstreamRequest_OllamaCloudBearer(t *testing.T) {
	...
	require.Equal(t, "Bearer ollama-cloud-key", ollamaReq.Header.Get("Authorization"))
	require.Empty(t, getHeaderRaw(ollamaReq.Header, "x-api-key"))
...
func TestOpenAIGatewayService_NativeAnthropicBridge_OllamaCloudBearer(t *testing.T) {
	...
	require.Equal(t, "ollama-cloud-key", getHeaderRaw(req.Header, "x-api-key"))
}
