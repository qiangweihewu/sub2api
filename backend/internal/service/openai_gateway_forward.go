...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	passthroughEnabled := account.IsOpenAIPassthroughEnabledWithContext(ctx)
	...
	for {
		...
		resp, err := s.doOpenAIUpstreamWithEffectiveConcurrency(ctx, upstreamReq, proxyURL, account)
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if isOpenAIResponsesCompactPath(c) {
			...
			req.Header.Set("session_id", isolateOpenAIUpstreamSessionIDWithUserSeed(apiKeyID, codexAccountIdentitySource(c, account), userSeed, compactSession))
		} else {
			req.Header.Set("accept", "text/event-stream")
		}
		if promptCacheKey != "" {
			isolated := isolateOpenAIUpstreamSessionIDWithUserSeed(apiKeyID, codexAccountIdentitySource(c, account), userSeed, promptCacheKey)
