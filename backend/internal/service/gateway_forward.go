...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (result *ForwardResult, err error) {
	...
	if shouldMimicClaudeCode {
		...
		rewrittenBody, injectTrace5221 = s.rewriteMessageCacheControlIfEnabled(ctx, body)
	...
	if isModelMismatchForSignature(originalModel, reqModel) {
		if filtered := FilterThinkingBlocksForRetry(body, reqModel); !bytes.Equal(filtered, body) {
	...
	// 重试循环
	// 注意：err 直接复用 Forward 的具名返回值（上游 0.2.1 把签名改成
	// (result *ForwardResult, err error)），此处不能再 var err error 重声明。
