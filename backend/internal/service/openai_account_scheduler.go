...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		// 并发槽位以 effectiveAccountConcurrency（含 group/fleet 默认值回退）为准，
		// 而非账号原始 Concurrency：Concurrency=0 表示"用默认值"，不是"不限并发"。
		freshConcurrency := s.service.effectiveAccountConcurrency(ctx, fresh)
		if freshConcurrency != s.service.effectiveAccountConcurrency(ctx, candidate.account) {
			release(result)
			result, attempted, acquireErr = s.tryAcquireOpenAIAccountSlot(ctx, fresh.ID, freshConcurrency, budget)
			if !attempted {
				continue
			}
			if acquireErr != nil {
