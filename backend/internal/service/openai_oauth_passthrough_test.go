...
func (u *httpUpstreamRecorder) Prewarm(_ context.Context, _ string, _ string, _ int64, _ int, _ *tlsfingerprint.Profile) error {
	return nil
}
...
func TestOpenAIGatewayService_APIKeyPassthrough_RebuildsUpstreamErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// 注意（fork 分歧）：上游原本还有一条 "upstream forbidden"（403 + text/html）
		// 用例，期望重建为 502。本 fork 额外有 isInfraLevelUpstream4xxResponse——
		// HTML/空 body 的 4xx 被判定为基础设施级故障并立即切号（failover），不会写下游响应，
		// 因此该用例在本 fork 不成立。该行为由 gateway_infra_4xx_test.go 覆盖。
		{
		...
		{
			// contentType 用 application/json（上游原为 text/plain）：本 fork 会把
			// text/plain 的 4xx 判为基础设施级故障走 failover，见上方分歧说明。
			// 这里仍保留“非结构化 body 也要净化重建”的原始断言意图。
