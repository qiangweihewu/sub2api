...
func TestAccountTestService_AdaptiveChatOnlyProvidersTestChatAndAnthropicEndpoints(t *testing.T) {
	...
	// fork 差异：setAnthropicAPIKeyAuthHeader 走 setHeaderRaw，按线缆小写 "x-api-key"
	// 写入 header map（Claude CLI 指纹保真），Header.Get 的规范化查找 "X-Api-Key"
	// 取不到。用 fork 的 getHeaderRaw 读取，断言语义不变。
...
func TestAccountTestService_AnthropicProtocolProbesNativeEndpointWithoutBetaQuery(t *testing.T) {
	...
	// fork 差异：同上，x-api-key 以原始小写键写入，需用 getHeaderRaw 读取。
	require.Equal(t, "sk-anthropic-test", getHeaderRaw(req.Header, "x-api-key"))
