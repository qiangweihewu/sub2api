		...
		t.Run(protocol.name, func(t *testing.T) {
			...
			for _, tc := range []struct {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					// fork: OAuth 伪装路径刻意不透传客户端 header（Parrot 对齐），用 API-key 路径覆盖 Accept-Encoding 透传。
					account := &Account{Platform: PlatformAnthropic, Type: AccountTypeAPIKey}
					req, _, err := svc.buildUpstreamRequest(context.Background(), c, account,
						[]byte(`{"model":"claude-sonnet-4-6","messages":[]}`), "test-token", "apikey", "claude-sonnet-4-6", false, false)
					require.NoError(t, err)
