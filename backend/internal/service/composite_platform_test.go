...
// ListByPlatform 由 fork 的启动期 TLS 指纹审计协程（NewGatewayService ->
// auditAnthropicOAuthTLSProfiles）调用；上游没有该协程，故上游用例的 stub
// 未实现它，内嵌的 nil AccountRepository 会导致 panic。返回空集即可。
func (r *compositeOwnershipAccountRepo) ListByPlatform(context.Context, string) ([]Account, error) {
	return nil, nil
}
