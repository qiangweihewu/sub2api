...
type gatewayForwardingSettingsResult struct {
	...
	clientDatelineNormalization                                                           bool
	cacheControlMode, claudeOAuthSystemPrompt, claudeOAuthSystemPromptBlocks              string
}
...
func (s *SettingService) getGatewayForwardingSettingsCached(ctx context.Context) gatewayForwardingSettingsResult {
	if cached, ok := gatewayForwardingCache.Load().(*cachedGatewayForwardingSettings); ok && cached != nil {
		if time.Now().UnixNano() < cached.expiresAt {
			return gatewayForwardingSettingsResult{
				...
				cacheControlMode:                 cached.cacheControlMode,
	...
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		...
		values, err := s.settingRepo.GetMultiple(dbCtx, []string{
			...
			SettingKeyRewriteMessageCacheControlMode,
		...
		if err != nil {
			...
			gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
				...
				cacheControlMode:                 s.defaultRewriteMessageCacheControlMode(),
				clientDatelineNormalization:      true,
				expiresAt:                        time.Now().Add(gatewayForwardingErrorTTL).UnixNano(),
			})
			return gatewayForwardingSettingsResult{openAITTFTMode: OpenAITTFTModeSemantic, fp: true, claudeOAuthSystemPromptInjection: true, rewriteMessageCacheControl: s.defaultRewriteMessageCacheControl(), cacheControlMode: s.defaultRewriteMessageCacheControlMode(), clientDatelineNormalization: true}, nil
		}
		...
		cacheControlMode := s.defaultRewriteMessageCacheControlMode()
		if v, ok := values[SettingKeyRewriteMessageCacheControlMode]; ok && v != "" {
			cacheControlMode = v
		}
		clientDatelineNormalization := true
