	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			if account.IsInterceptWarmupEnabled() {
				interceptType := detectInterceptType(body, reqModel, parsedReq.MaxTokens, parsedReq.Stream, isClaudeCodeClient)
			...
			if account.IsAnthropicOAuthOrSetupToken() && h.gatewayService.EffectiveAccountBaseRPM(c.Request.Context(), account) > 0 {
	...
	for {
		...
		for {
			...
			case config.UMQModeSerialize:
				...
				baseRPM := h.gatewayService.EffectiveAccountBaseRPM(c.Request.Context(), account)
			...
			if account.Platform == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
				...
				result, err = h.kiroGatewayService.Forward(requestCtx, c, account, body)
			} else {
			...
			if err != nil {
				...
				if errors.As(err, &promptTooLongErr) {
					...
					if !fallbackUsed && fallbackGroupID != nil && *fallbackGroupID > 0 {
						...
						if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), fallbackAPIKey.User, fallbackAPIKey, fallbackGroup, nil, service.PlatformFromAPIKey(fallbackAPIKey), reqModel); err != nil {
							status, code, message, retryAfter := billingErrorDetails(err)
							if retryAfter > 0 {
								c.Header("Retry-After", strconv.Itoa(retryAfter))
							}
...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	// 【注意】不计算并发，但需要校验订阅/余额
	if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(c.Request.Context(), apiKey), parsedReq.Model); err != nil {
		status, code, message, retryAfter := billingErrorDetails(err)
		if retryAfter > 0 {
			c.Header("Retry-After", strconv.Itoa(retryAfter))
		}
...
// fork: 仅拦截非流式探测（isStream=false）。mock 应答 sendMockInterceptResponse 走
// c.JSON 非流式格式，拦截流式请求会造成协议错配；流式探测（如 cc-switch v3.9.0+）放行到真实上游。
...
//   - isStream: 是否为流式请求（流式探测不拦截，见 isMaxTokensOneHaikuRequest）
