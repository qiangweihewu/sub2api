...
func createGroupRecord(ctx context.Context, client *dbent.Client, groupIn *service.Group) error {
	...
	builder := client.Group.Create().
		...
		SetDefaultAccountConcurrency(groupIn.DefaultAccountConcurrency).
		SetDefaultAccountRpm(groupIn.DefaultAccountRPM).
		SetDefaultPassthroughProfile(groupIn.DefaultPassthroughProfile).
		SetDefault429CooldownSec(groupIn.Default429CooldownSec).
