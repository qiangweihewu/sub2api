...
func updatedAccountRows(id int64, extra string) *sqlmock.Rows {
	...
	return sqlmock.NewRows(dbaccount.Columns).AddRow(
		...
		// 第 28/29 列是本 fork 独有的 temp_unsched_step_index /
		// temp_unsched_last_recovered_at（上游 account 表没有这两列）。
		service.StatusActive, nil, nil, nil, false, true, nil, nil, nil, nil, nil, 0, nil,
