  ...
  >
    ...
    >
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        ...
        <div v-if="!openaiBedrockMantle && (!isMultiProtocolPlatform || apiProtocol !== 'adaptive')">
      ...
      <div v-if="form.platform === 'anthropic' && accountCategory === 'bedrock'" class="space-y-4">
        ...
        <div>
          ...
          <select v-model="bedrockRegion" class="input">
            <optgroup v-for="grp in AWS_BEDROCK_REGIONS" :key="grp.label" :label="grp.label">
              <option v-for="r in grp.options" :key="r.code" :value="r.code">{{ r.code }} ({{ r.city }})</option>
            </optgroup>
          </select>
          <p class="input-hint">{{ t('admin.accounts.bedrockRegionHint') }}</p>
        </div>
...
import { AWS_BEDROCK_REGIONS, VERTEX_LOCATION_OPTIONS } from '@/constants/account'
import {
...
const submitCreateAccount = async (payload: CreateAccountRequest) => {
  ...
  try {
    const account = await adminAPI.accounts.create(withAntigravityConfirmFlag(applyUpstreamPassthroughToCreatePayload(payload)))
