...
// Fork: releases + install-custom.sh live in our own repo, not upstream's
const GITHUB_REPO = 'qiangweihewu/sub2api'
...
const scriptRollbackCommand = computed(() => {
  ...
  return `curl -sSL https://raw.githubusercontent.com/${GITHUB_REPO}/main/deploy/install-custom.sh | sudo VERSION=${tag} bash -s -- upgrade`
})
