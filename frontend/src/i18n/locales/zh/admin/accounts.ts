export default {
    accounts: {
      ...
      batchBedrock: {
        ...
        apiKeyHint: '账号级密钥，一把即可，将分发到所有选中的区域。',
        concurrency: '并发（每账号）',
        ...
        globalGroup: 'Global 分组',
        globalOnly: '仅 global',
        globalProfile: 'Global（global.）',
        group: '分组（账号池）',
        groupHint: '所有账号使用同一分组与相同优先级，网关会在各区域之间做负载均衡。',
        import: '导入 {count} 个',
        importing: '导入中…',
        invert: '反选',
        loadFactor: '负载权重',
        menuTitle: '批量导入 Bedrock（所有区域）',
        namePrefix: '名称前缀',
        noGroup: '— 无 —',
        noGroupWarning: '未选择分组 —— 这些账号将不会作为账号池进行负载均衡。',
        ...
        splitGroups: '将 Geo 与 Global 分入不同分组',
        title: '批量导入 Bedrock（所有区域）',
        willCreate: '将创建 {count} 个账号（{regions} 区域 × {profiles} 配置）。'
      },
      upstreamPassthrough: {
        advancedHelp: '默认显示「继承」。点击切换成「强制 ON」或「强制 OFF」。',
        advancedTitle: '高级（单 toggle override）',
        baseRpmHint: '0 表示不限。官方账号建议 ≤ 200，逆向账号 ≤ 50',
        category: '账号类别',
        categoryAuto: '自动 (推导: {derived})',
        categoryDerivedBadge: '推导',
        categoryOfficial: '官方直连 (official)',
        categoryOverrideLabel: '手改类别',
        categoryRelay: '中转 (relay)',
        categoryReverse: '逆向客户端 (reverse)',
        categoryReverseWarn: '⚠ 逆向客户端账号默认走 Strict 严格保护，请谨慎放宽',
        concurrencyHint: '中转账号一般 10+；官方账号 ≤ 3；逆向账号 ≤ 2',
        description: '在系统默认之上对该账号做精细化覆盖。',
        finalEffectiveCollapse: '收起',
        finalEffectiveExpand: '展开查看 7 toggle 实际值',
        finalEffectiveOverrideCount: '{count} 个覆盖',
        finalEffectiveTitle: '📍 最终生效',
        killSwitchActive: '⚠ 全局 kill switch 已激活，账号设置被覆盖',
        profileInherit: '继承类别默认 ({profile})',
        profileLabel: 'Profile',
        profileProtected: 'Protected（标准保护）',
        profileStrict: 'Strict（严格模仿）',
        profileTransparent: 'Transparent（全透传）',
        recommendedFor: '建议: {value} ({category})',
        title: '上游透传策略',
        tristateInherit: '继承',
        tristateOff: '强制 OFF',
        tristateOn: '强制 ON'
