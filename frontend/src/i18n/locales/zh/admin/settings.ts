export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        rewriteMessageCacheControlMode: '缓存注入模式',
        rewriteMessageCacheControlModeFull: 'Full（旧版）— 清除并重新注入代理断点',
        rewriteMessageCacheControlModeHint: 'safe（推荐）：仅在客户端无 cache_control 时注入，保留客户端原有设置。shadow：仅观测，不修改请求体，仅记录 trace。full（旧版）：清除客户端所有断点后重新注入代理断点。',
      ...
      upstreamPassthrough: {
        categoryDefaultsTitle: '按类别默认策略',
        categoryOfficial: '官方直连 (official)',
        categoryOfficialDesc: '官方 API Key/Service Account 直连——做标准卫生处理。',
        categoryRelay: '中转账号 (relay)',
        categoryRelayDesc: '指向其它聚合器/中转的账号——通常需要把客户端原样透传过去。',
        categoryReverse: '逆向客户端 (reverse)',
        categoryReverseDesc: 'Kiro / Antigravity / Claude Code / Codex 等模仿合法客户端的账号——必须严格保护以避免封号。',
        description: '按账号类别统一管理客户端到上游的透传/保护行为；可设置紧急 kill switch 与三类默认。',
        featureFlagEnabled: '启用上游透传策略 v1',
        featureFlagEnabledHint: '关闭时所有新策略代码路径均为 no-op，行为保持现状；开启后下面的设置才会生效，建议先在测试环境灰度。',
        globalOverrideAuto: '自动（按账号类别）',
        globalOverrideForceProtected: '强制 Protected',
        globalOverrideForceStrict: '强制 Strict（紧急封号防御）',
        globalOverrideForceTransparent: '强制 Transparent（调试用，谨慎）',
        globalOverrideHint: '非「自动」时会覆盖所有账号的本设置，仅用于排障与紧急封号防御。',
        globalOverrideTitle: '全局紧急开关 (kill switch)',
        profile: 'Profile',
        profileProtected: 'Protected（标准保护）',
        profileStrict: 'Strict（严格模仿）',
        profileTransparent: 'Transparent（全透传）',
        resetDefaults: '恢复推荐默认',
        title: '上游透传策略',
        toggles: {
          forwardBetaFlags: '透传 Beta 标识',
          forwardBetaFlagsDesc: '客户端 anthropic-beta / OpenAI-Beta 原样转发，跳过平台决定的 beta 注入（含 force_1m_context）。',
          forwardClientHeaders: '透传客户端请求头',
          forwardClientHeadersDesc: '开启后切换到「黑名单」模式（仅剥 Authorization / Cookie / Set-Cookie / Proxy-Auth / x-api-key），其它头一律放行。',
          forwardClientUA: '透传客户端 User-Agent',
          forwardClientUADesc: '上游收到客户端原始 UA，而不是平台 mimic 的 UA；sticky-session 哈希用归一化值不受影响。',
          forwardUserNetworkInfo: '透传用户网络信息',
          forwardUserNetworkInfoDesc: '放行 X-Forwarded-For / X-Real-IP / Forwarded / CF-Connecting-IP / True-Client-IP 等终端 IP 头。',
          skipBodyScrub: '跳过请求体净化',
          skipBodyScrubDesc: '跳过 ScrubThirdPartyBody / metadata_prune / tool name remap 等净化步骤。',
          skipModelRewrite: '跳过模型映射',
          skipModelRewriteDesc: '跳过账号级 model_mapping 应用；rate-limit 桶和计费仍按映射后名称记账。',
