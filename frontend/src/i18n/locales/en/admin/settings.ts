export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        rewriteMessageCacheControlMode: 'Cache Injection Mode',
        rewriteMessageCacheControlModeFull: 'Full (legacy) — strip and re-inject proxy breakpoints',
        rewriteMessageCacheControlModeHint: 'safe (recommended): only inject when client has no cache_control — preserves client settings. shadow: dry-run only, records trace without modifying the request body. full (legacy): strip all client breakpoints and re-inject proxy breakpoints.',
      ...
      upstreamPassthrough: {
        categoryDefaultsTitle: 'Per-category default profile',
        categoryOfficial: 'Official direct',
        categoryOfficialDesc: 'Vendor API Keys / Service Accounts — apply standard hygiene.',
        categoryRelay: 'Relay accounts',
        categoryRelayDesc: 'Accounts that point at another aggregator/relay — typically the safest place to forward the client verbatim.',
        categoryReverse: 'Reverse clients',
        categoryReverseDesc: 'Kiro / Antigravity / Claude Code / Codex impersonation accounts — must strictly mirror the impersonated client to avoid bans.',
        description: 'Manage client-to-upstream forwarding/protection per account category. Includes an emergency kill switch and three category defaults.',
        featureFlagEnabled: 'Enable Upstream Passthrough Policy v1',
        featureFlagEnabledHint: 'When off, every new policy code path is a no-op and behavior matches today. Turn on only after staging validation.',
        globalOverrideAuto: 'Auto (derive per account category)',
        globalOverrideForceProtected: 'Force Protected',
        globalOverrideForceStrict: 'Force Strict (emergency ban defense)',
        globalOverrideForceTransparent: 'Force Transparent (debug, use with care)',
        globalOverrideHint: 'Any value other than "auto" overrides every account-level setting. Use only for diagnostics or emergency ban defense.',
        globalOverrideTitle: 'Global emergency override (kill switch)',
        profile: 'Profile',
        profileProtected: 'Protected (standard hygiene)',
        profileStrict: 'Strict (full mimic)',
        profileTransparent: 'Transparent (full passthrough)',
        resetDefaults: 'Restore recommended defaults',
        title: 'Upstream Passthrough Policy',
        toggles: {
          forwardBetaFlags: 'Forward beta flags',
          forwardBetaFlagsDesc: 'Pass anthropic-beta / OpenAI-Beta through unchanged and skip platform-decided beta merging or injection (including force_1m_context).',
          forwardClientHeaders: 'Forward client headers',
          forwardClientHeadersDesc: 'Switches header copy to blacklist mode (strips only Authorization / Cookie / Set-Cookie / Proxy-Auth / x-api-key); everything else passes through.',
          forwardClientUA: 'Forward client User-Agent',
          forwardClientUADesc: 'Send the client\'s real UA upstream instead of the platform mimic UA; sticky-session hashing uses a normalized value regardless.',
          forwardUserNetworkInfo: 'Forward user network info',
          forwardUserNetworkInfoDesc: 'Lets X-Forwarded-For / X-Real-IP / Forwarded / CF-Connecting-IP / True-Client-IP reach the upstream.',
          skipBodyScrub: 'Skip body scrubbing',
          skipBodyScrubDesc: 'Bypass ScrubThirdPartyBody / metadata_prune / tool-name remap and other body hygiene steps.',
          skipModelRewrite: 'Skip model rewrite',
          skipModelRewriteDesc: 'Bypass account-level model_mapping on the upstream wire; rate-limit buckets and billing still key off the mapped name.',
