  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 class="text-xl font-bold text-gray-900 dark:text-white">Group Dashboard</h1>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Group #{{ groupId }} (all accounts combined)
          </p>
        </div>
        <div class="flex items-center gap-2">
          ...
          <ExportButton scope="group" :id="groupId" :from="from" :to="to" />
        </div>
      </div>

      <!-- Stats -->
      <StatsOverview scope="group" :id="groupId" :from="from" :to="to" />

      <!-- Account Breakdown (group-only) -->
      <AccountBreakdownTable :group-id="groupId" :from="from" :to="to" />

      <!-- IP Breakdown -->
      <IPBreakdownTable scope="group" :id="groupId" :from="from" :to="to" />
