...
async function createOrder(orderAmount: number, orderType: OrderType, planId?: number, options: CreateOrderOptions = {}) {
  ...
  try {
    ...
    // Map the visible method onto Stripe's payment_method_types.
    // - wxpay  → wechat_pay
    // - card   → card
    // - stripe → '' (let the landing page render the full Payment Element with card/link/alipay/wxpay)
    // - others → alipay
    const stripeMethod = (() => {
      if (visibleMethod === 'wxpay') return 'wechat_pay'
      if (visibleMethod === 'card') return 'card'
      if (visibleMethod === 'stripe') return ''
      return 'alipay'
    })()
